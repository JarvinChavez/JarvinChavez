# Jarvin Chavez

Full stack software engineer · AI systems · Houston, TX

**Python · TypeScript · React · Node.js · C++ · FastAPI · AWS**

B.S. Computer Science @ University of St. Thomas (2026)

---

## Featured projects

| Project | Stack | Description |
|---------|-------|-------------|
| [**Neta**](https://github.com/JarvinChavez/Neta) | C++ · Qt 6 · QML | Capstone — cross-platform desktop structured notebook |
| [**icn3d-implementation-proof-of-concept**](https://github.com/JarvinChavez/icn3d-implementation-proof-of-concept) | Node.js · Express · iCn3D | NSF research — motif rows → 3D protein visualization commands |
| [**document-ai-viewer**](https://github.com/JarvinChavez/document-ai-viewer) | FastAPI · Docker | Inspect Google Document AI JSON entities in a web UI |
| [**protein-motif-encoding**](https://github.com/JarvinChavez/protein-motif-encoding) | Python · OpenCV · pandas | UniProt motif tokenization + research visualization utilities |
| [**Data-science-assignments**](https://github.com/JarvinChavez/Data-science-assignments) | Python · scikit-learn | FARS crash contributing-factors pipeline |
| [**leetcode-solutions**](https://github.com/JarvinChavez/leetcode-solutions) | Python | Structured DSA study log — [progress](https://github.com/JarvinChavez/leetcode-solutions) |

---

## Small projects

Short solo scripts and experiments — [github.com/JarvinChavez/small-projects](https://github.com/JarvinChavez/small-projects)

Speech-to-text · tic-tac-toe minimax AI · anagram check · HTML table builder · Postgres seed script

---

## Interesting problems

Unfinished code, what broke, and how I'd solve it — [github.com/JarvinChavez/interesting-problems](https://github.com/JarvinChavez/interesting-problems)

Each folder: `attempt.*` (where I stopped) + `solution.*` (reference fix)

---

## Other work

| Project | Notes |
|---------|-------|
| [**something-will-work**](https://github.com/JarvinChavez/something-will-work) | Python/pygame game prototype |
| [**WeepingSouls**](https://github.com/JarvinChavez/WeepingSouls) | Godot rebuild (team project) |
| [**f22-python-proj-JarvinChavez**](https://github.com/JarvinChavez/f22-python-proj-JarvinChavez) | Intro to Programming (Fall 2022) |

Production code at **Kassandra (Helen)**, **Corva AI**, and **CareerSpring** is private — happy to discuss in interviews.

---

## Currently

- Founding engineer on **Helen** @ Kassandra Wellness Group
- Working through **100 LeetCode** problems
- Open to full-time and internship roles (SWE · AI systems · data engineering)

---

## Links

- [LinkedIn](https://www.linkedin.com/in/jarvin-chavez-b9ba9a266/)
- [Resume (PDF)](https://github.com/JarvinChavez/JarvinChavez/blob/main/resume.pdf)
